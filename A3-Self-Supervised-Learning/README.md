...
## Model
In this assignment, two self-supervised learning methods are studied: DINO and Masked Autoencoder (MAE). DINO is a self-distillation method that trains a student network to match the output of a teacher network under different augmented views of the same image. It uses global and local crops, teacher momentum updates, and centering to prevent representation collapse. MAE, on the other hand, learns by masking a large portion of image patches and training the model to reconstruct the missing parts. This encourages the model to understand image structure and visual patterns.

## Ablation
The assignment also includes ablation studies to analyze the effect of important design choices. For DINO, the experiments compare the default setting, a version without centering, and a version without local crops. These experiments help show how centering and multi-crop augmentation affect representation learning. For MAE, different mask ratios are tested to observe how the amount of hidden image information affects reconstruction quality and downstream classification performance.

The learned representations are evaluated using linear evaluation accuracy, training loss curves, and visualizations. The loss curves show how each model learns during training. The MAE reconstruction grid shows the original, masked, and reconstructed images to evaluate reconstruction quality. The DINO attention maps show which image regions the model focuses on across different attention heads. Finally, t-SNE visualization is used to compare the feature distributions learned by DINO and MAE, showing how well the representations separate CIFAR-10 classes.
